...
def generate_block(prev_block, transactions, difficulty):
    ...
    nonce = 0   # 初始化随机数
    while True:
        rand = random.randint(0, 100000) # 生成随机数
        block_str = (str(prev_block) + str(transactions) + str(nonce) + str(rand)) # 构造区块字符串
...
if __name__ == '__main__':
    ...
    block = generate_block(prev_block, transactions, difficulty)  # 生成区块
