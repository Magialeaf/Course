...
def rotate_right(a,k):
    k %= 32
    return ((a >> k) & 0xFFFFFFFF) | ((a & 0xFFFFFFFF) << (32 - k))
def rotata_shift(a,k):
    k %= 32
    return ((a >> k) & 0xFFFFFFFF)
def P_0(X):
    return (rotate_right(X,7) ^ (rotate_right(X,18))) ^ (rotata_shift(X,3))
def P_1(X):
    return (rotate_right(X, 17) ^ (rotate_right(X, 19))) ^ (rotata_shift(X, 10))
...
def CF(V_i,B_i):
    ...
    for j in range(0,64):
        ...
        ch = (E & F) ^ ((~E) & G)
        temp1 = (H + SS1 + ch + K[j] + W[j]) & 0xFFFFFFF
        maj = (A & B) ^ (A & C) ^ (B & C)
        temp2 = (SS0 + maj) & 0xFFFFFFF
        H = G
        G = F
        F = E
        E = (D + temp1)
...
def hash_msg(msg):
    ...
    for i in range(reserve1,56):
        msg1 += struct.pack("B",0)

    bit_length = (len1) * 8
    bit_length_string = struct.pack(">Q",bit_length)
    msg1 += bit_length_string
    print(len(msg1))
