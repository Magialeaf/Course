...
/*-----------------------------自定义函数-----------------------------*/
 //生成16轮的子密钥；
void SetKey(char My_key[9]);
//字节转换成位；
void ByteToBit(bool* Data_out, char* Data_in, int Num); 
//二进制的位置进行转换；
void Change_bit(bool* Data_out, int Num);
//位转换成字节；
void BitToByte(char My_message[9], bool* Message_in, int Num); 
  //各种表的置换算法；
void TableReplace(bool* Data_out, bool* Data_in, const char* Table, int Num);
 //二进制数组的拷贝;
void Bitcopy(bool* Data_out, bool* Data_in, int Num); 
 //左移位；
void Loop_bit(bool* Data_out, int movstep, int len); 
//des的轮加密算法
void Run_Des(char My_message[9], char HexMssage[16]);
//执行异或
void Xor(bool* Message_out, bool* Message_in, int Num); 
 // S盒变换；
void S_change(bool* Data_out, bool* Data_in); 
// 十六进制转二进制
void HexToBit(bool* Data_out, char* Data_in, int Num); 
 //二进制转换成十六进制；
void BitToHex(char* Data_out, bool* Data_in, int Num);
// DES轮解密算法；
void Run_desDes(char My_message[9], char HexMessage[16]);
...
{
    int i = 0, j;
    char My_key[9] = { 0 };  //记录加密密钥；
    char You_key[9] = { 0 }; //解密密钥
    char My_message[9] = { 0 }; //明文
    char Message_hex[17] = { 0 };//16进制的密文
...
void SetKey(char My_key[8])//————————————————————————————————————
{
    ...
    ByteToBit(Key_bit, My_key, 64);//字节变成位
...
void S_change(bool* Data_out, bool* Data_in) //S盒变换     S_change(Data_out, Message_E);                 // S盒变换
{
    int i;
    int r = 0, c = 0;//S盒的行和列；
...
void F_change(bool Data_out[32], bool Data_in[48])   // f函数;F_change(Message_bit_R, SubKey[i]);
{
    int i;
